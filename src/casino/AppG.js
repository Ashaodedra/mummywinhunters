...
export class AppG {
    ...
    static initGameConst() {
        ...
        if (!isNaN(_maxUrlAuto))
            _gameConst.getData("autoSpinNumber")[
            _gameConst.getData("autoSpinNumber").length - 1
                ] = _maxUrlAuto;
    ...
    static checkURL() {
        ...
        if (AppG.getURLParameter("jackpot"))
            _isHaveJackpot = AppG.getURLParameter("jackpot") === "true";
        if (AppG.getURLParameter("mute") && AppG.getURLParameter("mute") === "true")
            OMY.Omy.sound.mute();
        ...
        if (AppG.getURLParameter("locEditMode") && AppG.getURLParameter("locEditMode") === "true")
            _locMode = true;
        if (AppG.getURLParameter("maxUrlAuto"))
            _maxUrlAuto = AppG.getURLParameter("maxUrlAuto");
        if (AppG.getURLParameter("rtsMode"))
            _notLoadSounds = AppG.getURLParameter("rtsMode") === "true";
    ...
    static add2Loader(configArray, BASE_PATH) {
        for (let i = 0, l = configArray.length; i < l; ++i) {
            ...
            if (!AppG.isHaveJackpot && loadElement.hasOwnProperty("jpContent")
                && loadElement["jpContent"] === true) continue;
    ...
    static set languageUrl(value) {
        ...
        if (_urlLanguageList.hasOwnProperty(value)) {
            ...
            if (_gameConst.getData("languageIDs").indexOf(value) !== -1)
    ...
    static get winCredit() {
        if (_winValueOnCurrency)
            return OMY.OMath.roundNumber(_winCredit / _serverWork.creditType, 100);
        else
            return OMY.OMath.roundNumber(_winCredit, 100);
    }
    ...
    static get totalWinInSpin() {
        if (_winValueOnCurrency)
            return OMY.OMath.roundNumber(_serverWork.totalWinInSpin / _serverWork.creditType, 100);
        else
            return OMY.OMath.roundNumber(_serverWork.totalWinInSpin, 100);
    }

    static get totalRespinWin() {
        if (_winValueOnCurrency)
            return OMY.OMath.roundNumber(_serverWork.totalRespinWin / _serverWork.creditType, 100);
        else
            return OMY.OMath.roundNumber(_serverWork.totalRespinWin, 100);
    }
