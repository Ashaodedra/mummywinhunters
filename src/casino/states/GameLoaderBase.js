...
export class GameLoaderBase extends OMY.OContainer {
    ...
    _onLoadBootLoad2() {
        ...
        this._logoCanvas.setMixByName(this._logoCanvas.json["custom_a_name"], this._logoCanvas.json["a_loop"], 0.1);
    ...
    create() {
        ...
        if (AppG.locMode) {
            OMY.Omy.error("on localisation mode. pls use key 5 and 6 to change language. " +
                "End console method changeLoc(loc_const)");
    ...
    _continueAllLoaded() {
        ...
        const bitmapFontJson = OMY.Omy.assets.getJSON("BitmapFont");
        if (bitmapFontJson) {
            this._createBitmapFont(bitmapFontJson.main)
            this._createBitmapFont((OMY.Omy.isDesktop) ? bitmapFontJson.desktop : bitmapFontJson.mobile)
        }
